...
| `inspect_elements` | 返回树状节点、文本、布局盒和元数据。传入查找参数时，只保留命中节点和它的父级树。 | `clientId` 可选目标客户端 ID；`timeoutMs` MCP 等待超时；`limit` 最大节点数，默认 80；`includeHidden` 是否包含隐藏或零尺寸节点；`query` 搜索常见节点字段；`selector`、`id`、`testId`、`text`、`accessibilityLabel`、`className`、`role`、`tag` 用于缩小匹配范围。 | JSON 文本。包含 `count` 和树状 `tree`，节点包含文本、可见性、布局盒、测试 ID、无障碍标签、类名和子节点等运行环境可提供的信息。 |
